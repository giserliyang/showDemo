...
function createBlinkAnimation() {
    ...
    if (document.getElementById("blinkStyleFlag").selectedIndex === 0) {//频率
        goAnimation.blinkStyle = Cesium.BlinkAnimationBlinkStyle.Blink_Frequency;//频率闪烁
        goAnimation.blinkInterval = parseFloat(document.getElementById("blinkFrequency").value);//闪烁间隔
    } else {
        goAnimation.blinkStyle = Cesium.BlinkAnimationBlinkStyle.Blink_Number;//频率闪烁
    ...
    if (document.getElementById("b_replaceColorFlag").selectedIndex === 0) {
        goAnimation.replaceStyle =  Cesium.BlinkAnimationReplaceStyle.Replace_NoColor;
    } else {
        goAnimation.replaceStyle = Cesium.BlinkAnimationReplaceStyle.Replace_Color;//颜色交替类型
...
function createRotateAnimation() {
    ...
    if (document.getElementById("rotateDirectionFlag").selectedIndex === 0) {
        goAnimation.rotateDirection = Cesium.RotateDirection.ClockWise;//顺时针旋转

    } else {
        goAnimation.rotateDirection = Cesium.RotateDirection.AntiClockWise;//逆时针旋转
    }
